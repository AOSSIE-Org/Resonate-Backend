...
module.exports = async ({ req, res, log }) => {
  ...
  if (req.headers['x-appwrite-trigger']) {
    ...
    const index = meilisearch.index('users');
    ...
    try {
      ...
      if (eventType.includes('.delete')) {
        log(
          `Deleting document ${documentId} from Meilisearch due to deletion event`
        );
        ...
        log(
          `Syncing document ${documentId} to Meilisearch due to ${action} event`
        );
